...
from aspose.gis.formats.mapinfotab import MapInfoTabOptions
...
from aspose.gis.spatialreferencing import SpatialReferenceSystem, ParameterType
...
class Release_25_05_Tests(BaseTests):
    ...
    # Allow specifying SRS for MapInfo TAB layer.
    # https://issue.saltov.dynabic.com/issues/GISNET-650
    # https://issue.saltov.dynabic.com/issues/GISPYTHON-23
    def GISNET650Test(self):
        open_path = self.GetFileInBaseFolder("mercator.tab")
        create_path = self.GetFileInOutputFolder("mercator_create.tab")
    ...
    # Corrects X and Y coodinates disorder and adds Z in GML format.
    # https://issue.saltov.dynabic.com/issues/GISNET-1829
    # https://issue.saltov.dynabic.com/issues/GISPYTHON-25
    def GISNET1829Test(self):
        ...
        file_name = self.GetFileInOutputFolder("gml_xyz_bug.gml")
        ...
        if (not self.elements_equal(expected_doc, actual)):
